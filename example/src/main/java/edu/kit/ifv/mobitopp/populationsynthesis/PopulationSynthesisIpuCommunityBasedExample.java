...
public class PopulationSynthesisIpuCommunityBasedExample extends PopulationSynthesis {

	public PopulationSynthesisIpuCommunityBasedExample(SynthesisContext context) {
		super(context);
	}
	...
	protected DemandDataCalculator createCalculator(
		...
		steps.add(storeData());
		steps.add(cleanData());
		DemandDataForDemandRegionCalculator zoneCalculator = createZoneCalculator();
		return new DemandRegionDemandCalculator(regions(), zoneCalculator, steps,
				impedance());
	}

	private List<DemandRegion> regions() {
		return context().dataRepository().demandRegionRepository().getRegionsOf(RegionalLevel.community);
	}
	...
	private PopulationSynthesisStep workDestinationSelector(DemandRegionRelationsRepository communityRepository) {
		PanelDataRepository dataRepository = context().dataRepository().panelDataRepository();
		DemandRegionOdPairSelector communityPairCreator = DemandRegionOdPairCreator.forWork(communityRepository);
		ImpedanceIfc impedance = impedance();
		int range = context().experimentalParameters().valueAsInteger("poleDistanceRange");
		DemandRegionOdPairSelector odPairSelector = new PanelDistanceSelector(dataRepository, communityPairCreator,
				impedance, range);
		ZoneSelector zoneSelector = new DemandRegionBasedZoneSelector(communityRepository, newRandom());
  ...
  private DemandDataForDemandRegionCalculator createZoneCalculator() {
    ...
    Function<DemandRegion, Predicate<HouseholdOfPanelData>> householdFilter = r -> h -> true;
    WeightDemandCreatorFactory demandCreatorFactory = new WeightDemandCreatorFactory(
        ...
        householdFilter, householdSelector, dataRepository().zoneRepository());

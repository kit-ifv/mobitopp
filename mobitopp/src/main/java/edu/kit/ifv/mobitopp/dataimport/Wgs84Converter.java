...
public final class Wgs84Converter
{
  ...
  private final static double parseLatitude(
      String latitude_)
    throws ParseException
  {
    ...
    {
      throw new ParseException("illegal number format in '" + parts[0] + "' or '" + parts[1] + "'", -1);
    }
    ...
    {
      throw new ParseException("last character must be either 'N' or 'S'", -1);
    }
	}

	private static Object[] parseAllExceptTheLastCharacter(
			String latitude_, final MessageFormat input, final int lastChar) throws ParseException {
  ...
  private final static double parseLatitudeFromDecimal(
      String latitudeDecimal_)
    throws ParseException
  {
    ...
    {
      throw new ParseException("illegal number format in '" + latitudeDecimal_ + "'", -1);
    }

    if (result == Double.NaN)
    {
      throw new ParseException("illegal number format (NaN) in '" + latitudeDecimal_ + "'", -1);
    }
    
    return result;
  }

  private final static double parseLongitude(
      String longitude_)
    throws ParseException
  {
    ...
    {
      throw new ParseException("last character must be either 'W' or 'E'", -1);
    }
	}

  private final static double parseLongitudeFromDecimal(
      String longitudeDecimal_)
    throws ParseException
  {
    ...
    {
      throw new ParseException("illegal number format in '" + longitudeDecimal_ + "'", -1);
    }

    if (result == Double.NaN)
    {
      throw new ParseException("illegal number format (NaN) in '" + longitudeDecimal_ + "'", -1);
    }
  ...
  private static final Point2D getWorldPoint(
      double latitude,
      double longitude)
  ...
  public static Point2D getWorldPoint(
      String wgs84Coordinate_)
    throws ParseException
  {
	...
	/**
	 * Attention Normally latitude is given before longitude for coordinates. In this case
	 * it was requested to be the other way around.
	 * 
    ...
    catch (ParseException pe)
    {
      ...
      {
        throw new ParseException("illegal coordinate component in latitude: " + parts[0], -1);
      }
    }
		return latitude;
	}
	...
	private static double parseLongitude(Object[] parts) throws ParseException {
	  ...
	  catch (ParseException pe)
	  {
	    ...
	    {
	      throw new ParseException("illegal coordinate component in longitude: " + parts[1], -1);
	    }
	  }
		return longitude;
	}

...
public class DefaultPersonForSetup implements PersonBuilder {
	...
	public DefaultPersonForSetup(PersonId id, HouseholdForSetup household, int age,
			Employment employment, Gender gender, Graduation graduation, int income,
	...
	public boolean hasActivityOfTypes(Collection<ActivityType> activityTypes) {
		return getActivityPattern().asActivities().stream().map(Activity::activityType)
	...
	private NoSuchElementException missingDestination(ActivityType activityType) {
		return new NoSuchElementException(
				"No destination available for activity type: " + activityType);
	}
	...
	public Zone fixedActivityZone() {
		return fixedDestinations.getFixedDestination().map(FixedDestination::zone)
	...
	@Override
	public DefaultPersonForSetup setModeChoicePreferences(
	...
	public Person toPerson(Household household) {
		return new PersonForDemand(id, household, age, employment, gender, graduation, income,
				hasBike, hasAccessToCar, hasPersonalCar, hasCommuterTicket, hasDrivingLicense,
				getActivityPattern(), fixedDestinations, mobilityProviderMembership,
				modeChoicePrefsSurvey, modeChoicePreferences, travelTimeSensitivity);
	}

	@Override
	public PersonBuilder copy(PersonId id, HouseholdForSetup household) {
		DefaultPersonForSetup copy = new DefaultPersonForSetup(id, household, age, employment,

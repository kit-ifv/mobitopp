...
/**
 * Randomly draw from {@link WeightedHouseholds} considering the weight of the
 * {@link WeightedHousehold}s.
 * 
 * @author Lars Briem
 *
 */
...
public class ProbabilityBasedSelector implements WeightedHouseholdSelector {
	...
	public List<WeightedHousehold> selectFrom(List<WeightedHousehold> households, int amount) {
		...
		if (households.isEmpty()) {
			throw warn(new IllegalArgumentException("No households available to select from."),
				log);
		}

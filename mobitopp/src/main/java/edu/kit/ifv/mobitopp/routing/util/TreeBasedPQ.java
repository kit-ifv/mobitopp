...
public class TreeBasedPQ<T extends Comparable<T>>
	implements PriorityQueue<T> {

		
	private HashMap<T, PQElement<T>> mapping = new HashMap<T, PQElement<T>>();
	private TreeSet<PQElement<T>> data = new TreeSet<PQElement<T>>();


	public TreeBasedPQ<T> createNew() {
		return new TreeBasedPQ<T>();
	}


	public void clear() {
		mapping = new HashMap<T, PQElement<T>>();
		data = new TreeSet<PQElement<T>>();
	}
	...
	public T minElement() {
		...
		PQElement<T> element = data.first();
	...
	public T deleteMin() {
		...
		PQElement<T> toBeRemoved = data.first();

...
public class ModeAvailabilityModelAddingCarsharing extends BasicModeAvailabilityModel
	implements ModeAvailabilityModel {

	public ModeAvailabilityModelAddingCarsharing(ImpedanceIfc impedance) {
		super(impedance);
	}

	@Override
	public Set<Mode> availableModes(Person person, Zone currentZone, ActivityIfc previousActivity,
		...
		if (isAtHome(previousActivity)) {
			...
			if (previousActivity.isModeSet()
				...
				assert !currentZone
					.carSharing()
					.isFreeFloatingZone((CarSharingCar) person.whichCar());
			...
			} else if (person.hasDrivingLicense()
				&& currentZone.carSharing().isFreeFloatingCarSharingCarAvailable(person)) {
				choiceSet.add(StandardMode.CARSHARING_FREE);
			}
		}

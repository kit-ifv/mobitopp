...
public class VisumNetworkReader extends VisumBaseReader {
	...
	private final VisumFileReader visumReader;
	private File file;
	...
	public VisumNetworkReader(NetfileLanguage language) {
		this(language, new CachedVisumReader());
	}
	...
	public VisumNetwork readNetwork(File file, String ptSystemCode) {
		...
		Map<Integer, List<VisumConnector>> connectors = readConnectors(nodes, zones,
			transportSystems);
		...
		carSharingStations
			.put("Stadtmobil", Collections.unmodifiableMap(carSharingStationsStadtmobil));
		carSharingStations
			.put("Flinkster", Collections.unmodifiableMap(carSharingStationsFlinkster));
		...
		VisumNetwork network = new VisumNetwork(transportSystems, linkTypes, nodes, links, turns,
			zones, connectors, vehicleCombinations, ptStops, ptStopAreas, ptStopPoints, walkTimes,
			ptLines, ptLineRoutes, ptTimeProfiles, ptVehicleJourneys, areas, chargingFacilities,
			chargingPoints, carSharingStations, territories);
	...
	private Map<Integer, VisumLink> readLinks(Map<Integer, VisumNode> nodes,
		VisumTransportSystems transportSystems, VisumLinkTypes linkTypes) {
	...
	private Map<Integer, List<VisumTurn>> readTurns(Map<Integer, VisumNode> nodes,
	...
	private void assignTurnsToNodes(Map<Integer, VisumNode> nodes,
		Map<Integer, List<VisumTurn>> data) {
	...
	private Map<Integer, List<VisumConnector>> readConnectors(Map<Integer, VisumNode> nodes,
		Map<Integer, VisumZone> zones, VisumTransportSystems allSystems) {
	...
	private Map<Integer, VisumVehicleUnit> readVehicleUnits(VisumTransportSystems allSystems) {
		...
		Map<Integer, VisumVehicleUnit> vehicleUnits = new VisumVehicleUnitReader(language,
			allSystems).readVehicleUnits(content);
	...
	private Map<Integer, VisumVehicleCombination> readVehicleCombinations(
		...
		Map<Integer, VisumVehicleCombination> combinations = new VisumVehicleCombinationReader(
			language, units2combinations).readCombinations(combinationContent);
	...
	private Map<Integer, VisumPtStopArea> readPtStopAreas(Map<Integer, VisumNode> nodes,
	...
	private Map<Integer, VisumPtStopPoint> readPtStopPoints(Map<Integer, VisumNode> nodes,
		Map<Integer, VisumLink> links, Map<Integer, VisumPtStopArea> ptStopAreas,
		VisumTransportSystems allSystems) {
	...
	private Map<StopAreaPair, VisumPtTransferWalkTimes> readTransferWalkTimesMatrix(
		...
		return new VisumPtTransferWalkTimesReader(language, ptStopAreas, ptSystemCode)
			.readTransferWalkTimes(content);
	}
	...
	private void assignRoutesToLines(Map<String, VisumPtLine> ptLines,
		Map<String, VisumPtLineRoute> result) {
		result
			.values()
			.stream()
	...
	private void readPtLineRouteElements(Map<String, VisumPtLineRoute> ptLineRoutes,
	...
	private Map<Integer, VisumEdge> readEdges(Map<Integer, VisumPoint> points,

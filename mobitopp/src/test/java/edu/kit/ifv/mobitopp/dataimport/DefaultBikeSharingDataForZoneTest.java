...
public class DefaultBikeSharingDataForZoneTest {
	...
	void isNoBikeSharingArea() throws Exception {
		...
		DefaultBikeSharingDataForZone data = new DefaultBikeSharingDataForZone(serviceArea,
				companies());
	...
	void isBikeAvailableForPerson() throws Exception {
		...
		verify(bikeSharingCompany).isBikeAvailableFor(person);
	}

	private void configureBikeAvailable() {
		when(person.isMobilityProviderCustomer(bikeCompany)).thenReturn(true);
		when(bikeSharingCompany.isBikeAvailableFor(person)).thenReturn(true);
		lenient().when(bikeSharingCompany.bookBikeFor(person)).thenReturn(bike);
	}
	...
	void isNoBikeAvailableForPerson() throws Exception {
		...
		when(bikeSharingCompany.isBikeAvailableFor(person)).thenReturn(false);
		BikeSharingDataForZone data = buildAvailableBikeService();
	...
	void bookFreeBike() throws Exception {
		...
		verify(bikeSharingCompany).bookBikeFor(person);
	}

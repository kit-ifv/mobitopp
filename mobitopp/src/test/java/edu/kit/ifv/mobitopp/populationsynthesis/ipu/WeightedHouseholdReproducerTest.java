...
public class WeightedHouseholdReproducerTest {
  ...
  private PanelDataRepository panelData;
	...
	void multipliesHouseholdsByWeight() throws Exception {
		...
		WeightedHousehold someHousehold = createHousehold(attributes(), weight, 1);
		WeightedHousehold otherHousehold = createHousehold(attributes(), weight, 2);
	...
	void multipliesHouseholdsPerAttributeByWeight() throws Exception {
		...
		double weight = amount / 2.0d;
		WeightedHousehold someHousehold = createHousehold(attributes(), weight, 1);
		WeightedHousehold ansomeHousehold = createHousehold(attributes(), weight, 2);
		WeightedHousehold otherHousehold = createHousehold(attributes(), weight, 3);
		WeightedHousehold anotherHousehold = createHousehold(attributes(), weight, 4);
	...
	private WeightedHousehold createHousehold(Map<String, Integer> attributes, double weight, int number) {
		short year = 2000;
		...
		return new WeightedHousehold(id, weight, attributes, context, panelHousehold);
	}

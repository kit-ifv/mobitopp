...
class SpecificTypeSerializers(private val targetType: JavaType, private val serializer: JsonSerializer<*>) :
    ...
    override fun findSerializer(
        ...
        if (type != null && (type == targetType || targetType in type.interfaces)) {
            return serializer
        }
        return null
    }
...
class GenericKeyValueBuilder<T>(
    ...
    val loadFromSubmodules: Boolean,
    private var parameterizedType: JavaType? = null,
) {
     ...
     */
    constructor(parameterizedType: JavaType, wraps: Class<T>, default: Map<String, T> = emptyMap(),
                loadFromSubmodules: Boolean) : this(wraps, default, loadFromSubmodules) {
    ...
    fun getModule(): SimpleModule {
        return if (parameterizedType != null) {
            SimpleModule("KeyValueSerializer of $wraps").apply {
                setDeserializers(SpecificTypeDeserializers(parameterizedType!!, getDeserializer()))
                setSerializers(SpecificTypeSerializers(parameterizedType!!, getSerializer()))
            }
        } else {
            SimpleModule("KeyValueSerializer of $wraps").run {

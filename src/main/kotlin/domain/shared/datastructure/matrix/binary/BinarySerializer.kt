...
interface BinaryStandardDeserializer : BinaryDeserializer {
    override fun deserialize(path: Path): StandardMatrix {
        return path.inputStream().buffered().use {
            ...
            for (i in 0 until size) {
                zoneIds[i] = ZoneId(input.readInt().toLong()) // The established format is writing IDs (represented as long) as int, so we need to read them as int.
            }
    ...
    /*
    Since this format is the one using hash codes at position 0 it should also be the interface that provides easy access
    to the hash code.
     */

...
interface ZoneImpedance : Impedance {
    ...
    override fun distance(from: HasZoneId, to: HasZoneId, mode: Mode): Distance {
        return distance(from.zoneId, to.zoneId, mode)
    }

    override fun duration(from: HasZoneId, to: HasZoneId, mode: Mode, time: Time): Duration {
        return duration(from.zoneId, to.zoneId, mode, time)
    }
    ...
    fun duration(from: ZoneId, to: ZoneId, mode: Mode, time: Time): Duration = durationMetric(mode, time).evaluate(from, to)
}
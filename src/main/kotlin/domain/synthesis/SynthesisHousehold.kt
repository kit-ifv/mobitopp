...
class SynthesisHousehold<S : MinimumHouseholdAttributes, T : MinimumPersonAttributes> constructor(
    ...
    fun addMember(member: SurveyPerson<T>, copyLambda: (T) -> T) {
        members.add(SynthesisPerson(this, age = member.age, sex = member.sex, attributes = copyLambda(member
            .attributes)))
    }
     ...
     */
    fun addMembers(members: Collection<SurveyPerson<T>>, copyLambda: (T) -> T) = members.forEach { addMember(it, copyLambda) }

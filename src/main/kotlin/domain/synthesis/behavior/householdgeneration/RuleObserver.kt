...
class LowerBoundObserver(
    ...
    override val absoluteDifference: Double
        get() = (lowerBound - sum()).coerceAtLeast(0.0)
     ...
     */
    override fun optimize() {
        this.timesAssign((lowerBound / sum()))
    }
}

class UpperBoundObserver(
    name: String,
    observedIndex: Int,
    vectors: List<ScalableVector>,
    val upperBound: Int,
    ...
    override val absoluteDifference: Double
        get() = (sum() - upperBound).coerceAtLeast(0.0)

    override val relativeDifference: Double
        get() = absoluteDifference / upperBound
    override val quotientDifference: Double
        get() = TODO("Not yet implemented")

    override fun optimize() {
        this.timesAssign((upperBound / sum()))
    }
}
...
class IntervalObserver(
    ...
    override val absoluteDifference: Double
        get() {
            val sum = sum()
            val lb = (interval.first - sum).coerceAtLeast(0.0)
            val ub = (sum - interval.last).coerceAtLeast(0.0)

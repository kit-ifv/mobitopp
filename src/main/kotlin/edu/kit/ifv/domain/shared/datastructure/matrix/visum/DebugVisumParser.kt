...
import edu.kit.ifv.utils.files.decompressedBufferedReader
 ...
 *
 * Unlike lightweight parsers optimized for speed, [DebugVisumParser] performs
 * additional checks in order to produce precise and verbose diagnostics when
 * encountering malformed input. This makes it useful for debugging or validating
 * input data, at the cost of slower performance.
 *
 * In production scenarios with well-formed, automatically generated inputs,
 * a faster parser is typically preferred.
 *
 * @param path the path to the Visum file
 */
class DebugVisumParser(val path: Path) : VisumParser {

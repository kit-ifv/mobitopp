...
fun <K, V> Map<K, V>.sortByValues(comparator: Comparator<V>): Map<K, V> = this.toList()
    .sortedWith(compareBy(comparator) { it.second })
    .toMap()

fun <K, V> Map<K, V>.invertMap(): Map<V, List<K>> = this.entries
    .groupBy({ it.value }, { it.key })

fun <K, V> Map<K, Collection<V>>.flattenAndInvertMap(): Map<V, List<K>> = this.entries.flatMap { (k, v) ->
    v.map { it to k }
 ...
 */
fun Int.toIndex(): Int = if (sign == -1) {
    -this - 1
} else {
    this
}

inline fun <K, V> Iterable<K>.associateWithNotNull(valueSelector: (K) -> V?): Map<K, V> = this.mapNotNull { key ->
    valueSelector(
        key,
    )?.let {
        key to it
...
inline fun <K, V> Iterable<V>.associateByNotNull(keySelector: (V) -> K?): Map<K, V> = this.mapNotNull { value ->
    keySelector(
        value,
    )?.let {
        it to value

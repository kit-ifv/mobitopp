...
class RuleObserverTest : SynthesisTest() {
    ...
    fun ruleVectorization() {
        val rule1 = ZoneCheckRule<Any, ISurveyHousehold<out Any>>("hhsize == 1", 20) { it.size == 1 }
        val rule2 = ZoneCheckRule<Any, ISurveyHousehold<out Any>>("hhsize == 2", 10) { it.size == 2 }
        ...
        val rule3 = ZoneRule<Any, ISurveyHousehold<out Any>>("aged 10", -0) { it.members.count { it.age == 10 } }
    ...
    fun observerManipulation() {
        ...
        val rule = ZoneRule<Any, ISurveyHousehold<out Any>>("hhsize == 1", 20) { it.size }
